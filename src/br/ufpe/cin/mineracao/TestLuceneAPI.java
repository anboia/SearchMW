...
public class TestLuceneAPI {
	...
	static final int titleRepeatCount = 10;
...
//			processor.createDocumentLogicView(docsPath, docsPath+"processed/");
			...
			//Analyzer analyzer = new EnglishAnalyzer(new CharArraySet(new Vector(), true),new CharArraySet(new Vector(), true));
			
			/** COM STOP_WORD e COM STEMMING **/
			//Analyzer analyzer = new EnglishAnalyzer(StandardAnalyzer.STOP_WORDS_SET,new CharArraySet(new Vector(), true));
			...
			while(true){
				...
				System.out.println("# urls = "+urlsFromQuery.size());
				...
				CalculoResultado.iniciarCalculos(urlsFromQuery, TestStackOverflowResult.getStackOverflowResult( inputQuery), reader.numDocs());
